$(document).ready(function() {
	...
	/*
	// Загрузка файла
	$.each($(document.body).find('input[type="file"][name="image"], input[type="file"][name="file"]'), function(index, value) {
		var $input = $(this);
		var upload = {
			placeholder: {
				el: $input.closest('label').find('.label > span > span'),
				text: $input.closest('label').find('.label > span > span').html(),
			},
		};
		$input.on('change', function(e) {
			...
			if(path.length>0) {
				...
				if(filename.length>=20) {
					filename = filename.substr(0, (20-3))+' ...';
				}
				upload.placeholder.el.html(filename);
			} else {
				upload.placeholder.el.html(upload.placeholder.text);
			}
		});
	});
	*/
	...
	if(!isMobile.any() && window.innerWidth>=1240) {
		...
		$('#industries').ScrollMagicAnimate({
			triggerHook: 0.7,
			before: function() {
				var items = $(this);
				var chain_start = 0;
				var chain_iterator = 150;
				$.each(items.find('.industry-item .bg'), function(index, value) {
					$(this).css('visibility', 'hidden').css('will-change', 'visibility, transform, opacity');
					chain_start = index*chain_iterator;
					$(this).css({'-webkit-animation-delay': (chain_start/1000)+'s', 'animation-delay': (chain_start/1000)+'s'});
					$(this).css('will-change', 'visibility, transform, opacity');
					$(this).removeClass('animated');
				});
			}, 
			after: function() {
				var items = $(this);
				if(!items.hasClass('animationFinished')) {
					$.each(items.find('.industry-item .bg'), function(index, value) {
						var animEffect = 'fadeInUp';
						$(this).css('visibility', 'visible');
						$(this).addClass(animEffect).addClass('animated').css('will-change', 'auto');
					});
				}
			}
		});
